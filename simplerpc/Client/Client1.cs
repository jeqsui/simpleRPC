...
/// <summary>
/// Each heat generates random value for itself that can be negative.
/// </summary>
...
class Heat
{
	...
	private void func1(IService x)//IService x
	{
		//use service
		var rnd = new Random();
		...
		while (true)
		{
			try
			{
				var value = rnd.Next(-100, 100);
				log.Info(value);
				x.AddLiteralHeatUseVar(value);
				Thread.Sleep(2000);
			}
			catch (Exception e)
			{
